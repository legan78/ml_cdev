...
\*******************************************************************************************************/
    ...
    /// Default covariance type model
    int _GMM_::defaultCovModel = cv::EM::COV_MAT_SPHERICAL;
    ...
    * @param gmm Instance to copy.
    */
    _GMM_::_GMM_(const _GMM_& gmm)
    {
        _GMM_* other = const_cast<_GMM_*>(&gmm);
        *((cv::EM*)(this)) = *(dynamic_cast<cv::EM*>(other));
        *((MLTypeModel< Gaussian >*)(this)) = *(dynamic_cast<MLTypeModel< Gaussian >*>(other));

        covModel = gmm.covModel;        
    }
    ...
    */
    _GMM_::_GMM_(const cv::Mat& X, int _K, bool verb, int _maxIt, double _eps,  int covType)
    : cv::EM(_K, covType)
    { 
        covModel = covType;
        K = _K;
        ...
        this->train( X, log_likelihood, labels );
        
        pWeights = weights.clone();
        
        for(int k = 0; k < K; k++)
        {
            mixture[k].getMean() = means.row(k).clone();
            mixture[k].getCovariance() = covs[k].clone();
        }
        
        QEnd = cv::sum(log_likelihood)[0];
        ...
        if( verbose )
        {
            ...
            for(int k=0; k<K; k++)
            {
               std::cout << Nk[k]
                         << "\n"
                         << pWeights.at<double>(0,k)
                         << "\n" 
                         << mixture[k].firstMoment()
                         << "\n"
                         << mixture[k].secondMoment() 
                         << std::endl
                         << "*==*"
                         << std::endl; 
    ...
    */
    _GMM_& _GMM_::operator=(const _GMM_&gmm)
    { 
        _GMM_* other = const_cast<_GMM_*>(&gmm);
        *((cv::EM*)(this)) = *(dynamic_cast<cv::EM*>(other));
        *((MLTypeModel< Gaussian >*)(this)) = *(dynamic_cast<MLTypeModel< Gaussian >*>(other));
    ...
    * @return Likelihood.
    */
    double _GMM_::predict(const cv::Mat& x, const int& component)
    {
        cv::Mat probs;
        
        cv::EM::predict(x, probs);
        
        probs.convertTo(probs, CV_64FC1);
        
        return probs.at< double >(0, component);
    }
    ...
    */
    cv::Vec2f _GMM_::predict(const cv::Mat& x, cv::Mat* outYz)
    { 
        cv::Mat probs;
        
        if(!this->means.rows || !this->means.cols)
            std::cout << "The clasifier is not trained WTF!" << std::endl;
        
        cv::Vec2d p = dynamic_cast<cv::EM*>(this)->predict(x);
    ...
    */
    ...
    { 
        ...
        switch(covModel)
        {
            case cv::EM::COV_MAT_GENERIC:
            {
                beta = K*D*(D+1)/2;
                break;
             }
            case cv::EM::COV_MAT_DIAGONAL:
            {
                beta = K*D;
                break;
            }
            default: // cv::EM::COV_MAT_SPHERICAL
                beta = K;
        }
     ...
     * @param X Pointer to the data to do the initialization
     */
    void _GMM_::init(const cv::Mat* X)
    {
        /// Init parameters of the mixture
        mixture = std::vector< Gaussian >( K );
        pWeights  = cv::Mat::zeros(1, K, CV_64FC1);
     ...
     */
    void _GMM_::maximizationStep(const cv::Mat* X)
    {
